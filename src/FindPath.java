public class FindPath {
    ...
    // CHẠY PLS HÃY CHẠY PLS ABCDEFGHIJKLMNOPQRSTUVWXYZ
    // Instance variables
    Map pyramidMap;
    ...
    public DLStack<Chamber> path() {
        ...
        while (!path.isEmpty()) {
            Chamber currentChamber = path.peek(); // Peek at top of stack
            ...
            Chamber c = bestChamber(currentChamber); // Run no matter what even if treasure found until maxxed out treasure
            if (c != null) {
                path.push(c);
                c.markPushed();
            } else {
